...
class DeepONetCartesianProd(dde.maps.NN):
    ...
    def call(self, inputs, training=False):
        ...
        # Further encode
        x_func1 = self.geoNet2(mix_reduced) # Output: [ bs , hidden dim ]
        x_loc = self.outNet2(mix) # Output: [ bs , Npt , hidden dim , 2 ]

        # ss = x_loc.shape
        # x_loc = tf.reshape( x_loc , [ ss[0] , ss[1] , ss[2] , 1 ] )
        x_loc = tf.expand_dims( x_loc , axis=-1 )

        # Einsum
        x = tf.einsum("bh,bnhc->bnc", x_func1 , x_loc)
...
sub = '_GDON'+str(N_out_pt)+'_DistSplit'
...
net = DeepONetCartesianProd(
        [ geoNet , geoNet2 ], [ outNet , outNet2 ] , activation, "Glorot normal")
